...
class ChatMessagesRecord extends FirestoreRecord {
  ...
  // "itemstatus" field.
  String? _itemstatus;
  String get itemstatus => _itemstatus ?? '';
  bool hasItemstatus() => _itemstatus != null;

  // "itemspecs" field.
  FoodItemStruct? _itemspecs;
  FoodItemStruct get itemspecs => _itemspecs ?? FoodItemStruct();
  bool hasItemspecs() => _itemspecs != null;
  ...
  void _initializeFields() {
    ...
    _itemstatus = snapshotData['itemstatus'] as String?;
    _itemspecs = FoodItemStruct.maybeFromMap(snapshotData['itemspecs']);
  }
...
Map<String, dynamic> createChatMessagesRecordData({
  ...
  final firestoreData = mapToFirestore(
    <String, dynamic>{
      ...
      'itemspecs': FoodItemStruct().toMap(),
    }.withoutNulls,
  );

  // Handle nested data for "itemspecs" field.
  addFoodItemStructData(firestoreData, itemspecs, 'itemspecs');
...
class ChatMessagesRecordDocumentEquality
  ...
  bool equals(ChatMessagesRecord? e1, ChatMessagesRecord? e2) {
    return e1?.user == e2?.user &&
        ...
        e1?.image == e2?.image &&
        e1?.video == e2?.video &&
        e1?.itemstatus == e2?.itemstatus &&
        e1?.itemspecs == e2?.itemspecs;
  }
